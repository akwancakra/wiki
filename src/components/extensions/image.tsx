  ...
  addCommands() {
    return {
      ...
      setImage:
        (options) =>
        ({ commands }) => {
          // If the image is a data URL (uploaded image), save it to public folder
          if (options.src && options.src.startsWith("data:")) {
            const fileName = options.alt
              ? `${options.alt
                  .replace(/\s+/g, "-")
                  .toLowerCase()}-${uuidv4().slice(0, 8)}.jpg`
              : `image-${uuidv4().slice(0, 8)}.jpg`;

            // Save image to public folder
            saveImageToPublic(options.src, fileName)
              .then((publicPath) => {
                // Update image source to the public path
          ...
          // For non-data URL images, use the default behavior
